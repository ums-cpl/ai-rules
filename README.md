...
└── rules/
    ├── global/
    │   └── coding-standards.md    # Typography, language, editing policy (source-file globs)
    └── tech/
        ├── cpp.md           # C++ style (applies when .cpp files are open)
        ├── latex.md         # LaTeX formatting (applies when .tex / .ltx files are open)
...
- **rules/global/** — Cross-cutting rules (not necessarily always-on; see each file's frontmatter).
...
| Rule | Description |
|------|-------------|
| global/coding-standards | Typography (Title Case), language (American English, code in English), deterministic editing; auto-attached for common source extensions |
| tech/cpp | *C++* style and conventions (placeholder; auto-attached for `**/*.cpp`) |
| tech/latex | *LaTeX* formatting (dashes, one sentence per line; auto-attached for `**/*.{tex,ltx}`) |
